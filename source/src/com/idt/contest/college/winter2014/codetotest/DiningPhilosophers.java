 ...
 * /wiki/Dining_philosophers_problem#Resource_hierarchy_solution
 * 
 * *** BUG IN THIS CLASS *** This class experiences a race condition because
 * there is no synchronization between threads when accessing the forkStates
 * array. Two Philosophers will check a forks availability, see that it is
 * available, and on the next processor iteration they will both pick up the
 * same fork. This race condition issue is timing based and is not guaranteed to
 * occur in the same order, or even at all.
 * 
 * 
 */
public class DiningPhilosophers {
	enum State {
		THINKING, EATING, WAITING_FOR_LOWER_FORK, WAITING_FOR_HIGHER_FORK, RELEASING_FORKS
	};

	/**
	 * integer array representing which philosopher has which fork (index
	 * represent fork, value represents philosopher)
	 */
	private static int[] forkStates;

	/**
	 * map to digest result and keep track of number of times eaten (key) and
	 * number of philosophers (value)
	 */
	 ...
	 * 
	 * @param numberOfPhilosphers
	 *            - number of philosophers at the party
	 * @param numberOfTimesToEat
	 *            - number of times they are expected to eat at the party
	 */
	public String execute(int _numberOfPhilosophers, int _numberOfTimesToEat) {
		...
		try {
			Thread.sleep(FrameworkConstants.TIME_TO_WAIT
					* _numberOfPhilosophers * _numberOfTimesToEat);
		...
		for (Integer i : numOfTimesEatenMap.keySet()) {
			resultString += numOfTimesEatenMap.get(i) + " philosophers ate "
					+ i + " times successfully\n";
		}
		return resultString;
	}

	/**
	 * class representing a single philosopher that will come to dinner, think
	 * and eat Since each philosopher is on their own thread, they can think and
	 * eat simultaneously
	 */
	private class Philosopher extends Thread {
		 ...
		 * 
		 * @param _identity
		 *            - identity of this philosopher
		 * @param _numberOfTimesToEat
		 *            - number of times they are expected to eat
		 * @param _numberOfForks
		 *            - number of forks on the table
		 */
		Philosopher(int _identity, int _numberOfTimesToEat) {
			...
			this.attemptsBeforeQuitting = _numberOfTimesToEat
					* _numberOfTimesToEat;
			...
			this.leftFork = determineLeftForkValue(_identity,
					DiningPhilosophers.forkStates.length);
			holdingForks = false;
			this.state = DiningPhilosophers.State.THINKING;
		}

		/**
		 * This is the code body that represents a philosophers dinner party
		 * experience The philosopher will loop between states until they have
		 * eaten enough or quit from frustration THINKING -> WAITING FOR LOWER
		 * FORK -> WAITING FOR HIGHER FORK -> EATING -> RELEASING FORKS
		 */
		@Override
		public void run() {
			...
			while ((this.numberOfTimesEaten < this.numberOfTimesToEat || holdingForks)
				...
				if (state == DiningPhilosophers.State.THINKING) {
					System.out.println(formatActionStatement(this.identity,
							"thinking"));
					...
					// represents the philosopher waiting for lower value fork
					// state
				} else if (state == DiningPhilosophers.State.WAITING_FOR_LOWER_FORK) {
					// if the lower fork is free, take it
					if (isForkAvailable(
							determineLowestValue(this.leftFork, this.rightFork),
							DiningPhilosophers.forkStates)) {
						System.out.println(formatActionStatement(
								this.identity,
								"picking up lower fork #"
										+ determineLowestValue(this.leftFork,
												this.rightFork)));
						pickUpFork(
								this.identity,
								determineLowestValue(this.leftFork,
					...
					// representing the philosopher waiting for higher value
					// fork state
				} else if (state == DiningPhilosophers.State.WAITING_FOR_HIGHER_FORK) {
					// if the higher fork is free, take it
					if (isForkAvailable(
							determineHighestValue(this.leftFork, this.rightFork),
							DiningPhilosophers.forkStates)) {
						System.out.println(formatActionStatement(
								this.identity,
								"picking up higher fork #"
										+ determineHighestValue(this.leftFork,
												this.rightFork)));
						pickUpFork(
								this.identity,
								determineHighestValue(this.leftFork,
							...
							"releasing fork #"
									+ determineLowestValue(this.leftFork,
											...
											+ determineHighestValue(this.leftFork,
													this.rightFork)));
					// releasing the forks
					releaseForks(this.leftFork, this.rightFork,
							DiningPhilosophers.forkStates);
					holdingForks = false;
			...
			// in case the Philosopher gets up and tries to walk away with a
			// fork out of frustration,
		 ...
		 * 
		 * @param _left
		 *            - first int to consider
		 * @param _right
		 *            - second int to consider
		 ...
		 * Generally, this is the index minus 1, but must handle wrapping if
		 * index is 0
		 * 
		 * @param _id
		 *            - int representing current philosophers index
		 * @param _numOfForks
		 *            - int representing total number of forks on table
		...
		/**
		 * Method used to format the philosophers id and an action into an
		 * action statement
		 * 
		 * @param _id
		 *            - id of this philosopher
		 * @param _verb
		 *            - action to add to action statement
		 * @return - String representation of philosopher and the action they
		 *         performed
		 */
		private String formatActionStatement(int _id, String _verb) {
			String actionStatement = "Philosopher #" + this.identity + " is "
					+ _verb + "...";
		...
		/**
		 * Method that allows DiningPhilosophers to retrieve number of times
		 * that this philosopher ate at the end of the dinner party
		 * 
		 * @return int number of times that this philosopher at at the dinner
		 *         party
		 */
		 ...
		 * 
		 * @param _forkIndex
		 *            - identity of fork to check availability
		 * @param _forks
		 *            - int array representing fork states
		 * @return - true if fork is available, false if fork is unavailable
		 */
		private boolean isForkAvailable(int _forkIndex, int[] _forks) {
			
			ThreadTester.createStateTrackerOverride(
					Thread.currentThread().getName(),
					ThreadTester.getState("forks"));
		 ...
		 * 
		 * @param _identity
		 *            - identity of philosopher
		 * @param _forkIndex
		 *            - index of fork to pick up
		 * @param _forks
		 *            - int array representing fork states
		...
		private boolean pickUpFork(int _identity, int _forkIndex, int[] _forks) {
			...
			ThreadTester.compareStates(currentThreadState,
					ThreadTester.getState(Thread.currentThread().getName()),
					"com.idt.contest.college.winter2014.codetotest.DiningPhilosophers pickUpForks");

			if (_forks.length >= _forkIndex + 1) {
				ThreadTester.compareWithGlobalState("forks", currentThreadState,
						"com.idt.contest.college.winter2014.codetotest.DiningPhilosophers pickUpForks");
		 ...
		 * 
		 * @param _primaryForkIndex
		 *            - primary index of fork to release
		 * @param _secondaryForkIndex
		 *            - secondary index of a fork to release
		 * @param forks
		 *            - int array of fork states
		...
		private boolean releaseForks(int _primaryForkIndex,int _secondaryForkIndex, int[] _forks) {
			...
			ThreadTester.compareStates(currentThreadState,ThreadTester.getState(
					Thread.currentThread().getName()),
